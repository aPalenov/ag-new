...
const compoundVariants = buttonConfig?.compoundVariants || []

const result = compoundVariants.reduce<Record<string, string[]>>(
  (acc, item) => {
    const variant = item.variant
    acc[variant] = acc[variant] || []

    if (!acc[variant].includes(item.color)) {
      acc[variant].push(item.color)
    }
    return acc
  },
